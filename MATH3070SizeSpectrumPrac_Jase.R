...
SST_nc <- open.nc("cesm_rcp85_temp_zs_annual_200601-210012.nc") # Sea surface temperature (SST)
...
Lons <- ifelse(Lons > 180, -360 + Lons, Lons) + 0.5 # Plots better with -180:180
...
sdf <-rasterFromXYZ(df[,c(1,2,3)], crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")  #Convert first two columns as lon-lat and third as value
...
st_crs(sf) <- 4326 # Set base projection

sf_rob <- st_transform(sf, crs = st_crs("+proj=robin +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")) # Alter the CRS to robinson

sf_moll <- st_transform(sf, crs = st_crs("+proj=moll +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs")) # Alter the CRS to robinson
...
# Plot SST for now
ggplot(data = sf_moll, aes(fill = SSTnow), colour = NA) +
  geom_sf() +
